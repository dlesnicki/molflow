...
!                  %----------------%
 ...
 !                 %----------------%
  ...
  do k=1,2
        ...
        filename_out="Gcom-"//trim(adjustl(suffix))//".dat"
        write(*,*) "####"//trim(filename_out)//"#####"

        open(unit=trajunit_out,file=trim(filename_out),status='unknown')
...
!                        %----------------%
 ...
 !                       %------------------%
        ...
        !G barbar
        call write_G(trajunit_out,Gcom,"com")
        close(12)
        !!Density Map
