...
!                  %----------------%
  ...
  character(len=200) :: filename_pos
  character(len=200) :: filename_vel
  type(xyzfile) :: trajfile_pos
  type(xyzfile) :: trajfile_vel
  integer :: trajunit_pos
  integer :: trajunit_vel
...
!                  %----------------%
  ...
  namelist /data/ filename_pos,filename_vel,nskips,nprint,a1,a2,a3,referential
  ...
  trajunit_pos=11
  call open_read_xyz(trajunit_pos,trim(filename_pos),trajfile_pos,natoms,iostat)

  trajunit_vel=12
  call open_read_xyz(trajunit_vel,trim(filename_vel),trajfile_vel,natoms,iostat)

  write(*,*) "---------------------"
  write(*,*) "file positions =",trim(filename_pos)
  write(*,*) "file velocities =",trim(filename_vel)
  write(*,*) "DATA INPUT :"
  write(*,*) "nskips =",nskips
  write(*,*) "nprint =",nprint
  write(*,*) "limit =",limit
...
!  call create_msd(msdAr,nbins=nbins,dt=dt)
  ...
  call read_xyz_config(trajfile_pos,configuration,iostat)
...
!                  %----------------%
     ...
     do skip=1,nskips
        call read_xyz_config(trajfile_pos,configuration,iostat)
        call read_xyz_config(trajfile_vel,velocities,iostat)
